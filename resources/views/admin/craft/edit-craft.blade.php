<x-admin-master>
  @section('content')
    <div class="container">
      <div class="col-8">
        <h1 class="text-center mb-3">Edit Craft Recipe</h1>
        <hr>

        <form action="{{ route('craft.update', $craft->id) }}" method="POST">
          @csrf
          @method('PATCH')
          {{-- Name input field --}}
          <div class="mb-3">
            <label class="form-label" for="name">Recipe name: <i class="fa-light fa-asterisk text-danger"></i></label>
            <input class="form-control {{$errors->has('name') ? 'is-invalid' : ''}}" type="text" name="name" value="{{ $craft->name }}">
          ...
          <div class="mb-3">
            ...
            <select class="form-control {{ $errors->has('profession_id') ? 'is-invalid' : '' }}" name="profession_id">
              ...
              <option value="{{ $prof->id }}" {{ $craft->profession_id == $prof->id ? 'selected' : '' }}>{{ $prof->name }}</option>
              @endforeach
            </select>
          ...
          <div class="mb-3">
            ...
            <input class="form-control {{$errors->has('wowhead_link') ? 'is-invalid' : ''}}" type="text" name="wowhead_link" value="{{ $craft->wowhead_link }}">
          ...
          <div class="mb-3">
            ...
            <select class="form-control {{$errors->has('quality') ? 'is-invalid' : ''}}" type="text" name="quality">
              ...
              <option value="1" {{ $craft->quality == 1 ? 'selected' : '' }}>1 Star</option>
              <option value="2" {{ $craft->quality == 2 ? 'selected' : '' }}>2 Star</option>
              <option value="3" {{ $craft->quality == 3 ? 'selected' : '' }}>3 Star</option>
              <option value="4" {{ $craft->quality == 4 ? 'selected' : '' }}>4 Star</option>
              <option value="5" {{ $craft->quality == 5 ? 'selected' : '' }}>5 Star</option>
            </select>
          ...
          <div class="mb-3">
            ...
            <input class="form-control {{$errors->has('comment') ? 'is-invalid' : ''}}" type="text" name="comment" value="{{ $craft->comment }}">
          ...
          <div class="d-flex">
            <div class="mx-auto">
              <a href="{{ route('craft.index') }}" class="btn btn-outline-danger">Cancel</a>
              <input type="submit" value="Update Craft Recipe" class="btn btn-success">
            </div>
          </div>
          {{-- End of Form Buttons --}}
        </form>
