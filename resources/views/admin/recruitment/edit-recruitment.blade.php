  ...
  @section('content')
    <div class="container">
      <div class="col-8">
        ...
        {{-- Recruitment Form --}}
        <form action="{{ route('recruit.update', $recruitment->id ) }}" method="POST">
        @csrf
        @method('PATCH')
        ...
        <div class="mb-2">
          ...
          <select class="form-control {{ $errors->has('playable_class_id') ? 'is-invalid' : '' }}" name="playable_class_id">
            ...
            @foreach ($classes as $class)
              <option value="{{ $class->id }}" {{ $recruitment->playable_class_id == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
            @endforeach
          </select>
        ...
        <div class="mb-2">
          ...
          <input class="form-control {{ $errors->has('comment') ? 'is-invalid' : '' }}" type="text" name="comment" value="{{ $recruitment->comment }}">
        ...
        <div class="mb-2 form-group">
          ...
          <select class="form-control {{ $errors->has('priority') ? 'is-invalid' : '' }}" name="priority">
            ...
            <option value="0" {{ $recruitment->priority == 0 ? 'selected' : '' }}>Low</option>
            <option value="1" {{ $recruitment->priority == 1 ? 'selected' : '' }}>High</option>
          </select>
        ...
        <div class="mb-2 form-group form-check">
          <input class="form-check-input {{ $errors->has('is_active') ? 'is-invalid' : '' }}" 
                  type="checkbox" 
                  name="is_active"
                  {{ $recruitment->is_active == 1 ? 'checked' : ''}}>
        ...
        <div class="d-flex">
          <div class="mx-auto">
            ...
            <button class="btn btn-success" type="submit">Update Recruitment</button>
          </div>
        </div>
        </form>
